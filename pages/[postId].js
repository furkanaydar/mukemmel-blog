...
class CurrentPost extends React.Component {
  ...
  async postComment(name, details) {
    ...
    await fetch(window.location.protocol + '://' + process.env.host + '/api/post/' + this.props.post.id + '/makeComment', settings);
...
CurrentPost.getInitialProps = async ({ req, query }) => {
  ...
  const posts_res = await fetch(window.location.protocol + "://" + process.env.host + "/api/posts");
  const posts_json = await posts_res.json();

  console.log(posts_json)
  const res = await fetch(window.location.protocol+ `://` + process.env.host + `/api/post/${query.postId}`);
  const comments = await fetch(window.location.protocol + '://' + process.env.host + '/api/post/' + query.postId + '/comments')
